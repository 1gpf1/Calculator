...
public  class Cal2 extends JFrame implements ActionListener,ItemListener
{
	...
	String []button= {"%","C","Backspace","/","π","7","8","9","*","1/X","4","5","6","-","XˆY","1","2","3","+","X^2","+/-","0",".","=","sqrt"};
...
jmb = new JMenuBar();
jm =new JMenu("选择");
rb1 = new JRadioButton("标准型");
rb2 = new JRadioButton("科学型",true);
...
//按钮
panel4 = new JPanel();
panel4.setLayout(new GridLayout(5,6,1,1));
for(int i=0;i<button.length;i++)
{ JButton an=new JButton(button[i]); //按钮添加
an.addActionListener(this);  //为按钮添加事件事件
panel4.add(an);
          ...
          //1/X运算
            ...
            //开根号运算
            	if(number < 0) {
            		//操作不合法
            		bool=false;
            		wbk1.setText("负数没有平方根");
            	}
            	else
            	{
            		number=Math.sqrt(getNumberFromText());
            	}
	        }else if(operator.equals("X^2")) {
        	//平方运算
	        	number*=number;
        	}else if(operator.equals("XˆY")) {
        	//幂运算
        		number=Math.pow(number,getNumberFromText() );
